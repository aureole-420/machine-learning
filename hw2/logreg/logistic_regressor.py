...
class LogisticRegressor:
    ...
    def train(self,X,y,num_iters=400):
        ...
        self.theta = theta_opt # assign value to theta
        return theta_opt
    ...
    def loss(self, *args):
        ...
        ##########################################################################
        gz = utils.sigmoid(X.dot(theta))
        J = (1.0 / m) * (- y.dot(np.log(gz).T) - (1-y).dot(np.log(1-gz).T))
    ...
    def grad_loss(self, *args):
        ...
        grad = (1.0 / m) * (utils.sigmoid(X.dot(theta)) - y).T.dot(X)
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return grad
    ...
    def predict(self, X):
        ...
        # y_pred = np.array([1 if (utils.sigmoid(X[i].dot(self.theta)) >= 0.5) else 0 for i in xrange(X.shape[0])])
        y_pred[utils.sigmoid(X.dot(self.theta)) >= 0.5] = 1
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return y_pred
...
class RegLogisticRegressor:
    ...
    def loss(self, *args):
        ...
        ##########################################################################
        gz = utils.sigmoid(X.dot(theta))
        J = (1.0 / m) * (- y.dot(np.log(gz).T) - (1-y).dot(np.log(1-gz).T)) + (0.5*reg/m) * np.sum(np.square(theta[1:]))
    ...
    def grad_loss(self, *args):
        ...
        # TODO: 1 line of code expected                                          #
        ##########################################################################
        grad = (1.0 / m) * (utils.sigmoid(X.dot(theta)) - y).T.dot(X) + (1.0 * reg/ m) * np.hstack([np.array([0]), theta[1:]])

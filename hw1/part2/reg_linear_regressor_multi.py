...
class RegularizedLinearRegressor_Multi:
    ...
    def normal_equation(self,X,y,reg):
        ...
        theta_n = np.linalg.inv(X.transpose().dot(X)).dot(X.transpose()).dot(y)
...
class RegularizedLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
    ...
    def loss (self,*args):
        ...
        ###########################################################################
        v = X.dot(theta) - y
        J = (v.T.dot(v) + reg * theta[1:].dot(theta[1:])) / len(y) * 0.5
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return J
    ...
    def grad_loss(self,*args):                                                                          
        ...
        ###########################################################################
        grad = X.transpose().dot(X.dot(theta) - y) / len(y)
        grad[1:] +=  reg * theta[1:] / len(y)
